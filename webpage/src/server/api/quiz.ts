...
import { saveQuizCompletionToPuzzleDay } from "../services/firebase-quiz-completion.service";
...
function getLegacyPuzzleId(questionId: string): string | null {
  const match = /^(.*)_q\d+$/i.exec(questionId.trim());
  if (!match) return null;
  const puzzleId = match[1]?.trim();
  return puzzleId ? puzzleId : null;
}
...
export async function handleSubmitQuiz(req: Request): Promise<Response> {
  ...
  if (owner.userId && !owner.isAdmin) {
    const legacyPuzzleId = getLegacyPuzzleId(todayQuestion.id);
    completionResult = await recordPlayerQuizCompletion({
      ...
      ...(legacyPuzzleId ? { puzzleId: legacyPuzzleId } : {}),
    });
    ...
    if (legacyPuzzleId) {
      try {
        ...
        console.error("[quiz] failed to save puzzle-day completion", {
          questionId: todayQuestion.id,
          legacyPuzzleId,
          userId: owner.userId,
          error,
