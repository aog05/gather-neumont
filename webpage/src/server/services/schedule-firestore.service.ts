...
export async function setScheduleEntry(
  ...
  const scheduledEntries = await listScheduleEntries();
  const existingDates = scheduledEntries
    .filter(
      (entry) =>
        entry.questionId === normalizedQuestionId &&
        entry.dateKey !== normalizedDateKey
    )
    .map((entry) => entry.dateKey);
  if (existingDates.length > 0) {
    existingDates.sort((a, b) => a.localeCompare(b));
    return {
      error: "question_already_scheduled",
      code: "QUESTION_ALREADY_SCHEDULED",
      message: `Question ${normalizedQuestionId} is already scheduled on ${existingDates.join(", ")}.`,
      existingDates,
    };
  }
