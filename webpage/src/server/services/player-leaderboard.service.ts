...
export async function recordPlayerQuizCompletion(
  ...
  return runTransaction(db, async (tx) => {
    ...
    const existingPuzzleRecord = Array.isArray(playerData.PuzzleRecord)
      ? playerData.PuzzleRecord.filter(
          (entry): entry is string => typeof entry === "string" && entry.trim().length > 0
        )
      : [];
    const nextPuzzleRecord =
      typeof input.puzzleId === "string" && input.puzzleId.trim().length > 0
        ? [...new Set([...existingPuzzleRecord, input.puzzleId.trim()])]
        : existingPuzzleRecord;
