...
interface QuarantinedQueueProps {
...
export default function QuarantinedQueue(props: QuarantinedQueueProps) {
  const { messages, loading, error, onSelectMessage } = props;

  if (loading) return <p className="forum-status-text">Loading quarantined queue...</p>;
  if (error) return <p className="lb-error">{error}</p>;
  if (messages.length === 0) return <p className="forum-status-text">No quarantined messages.</p>;
    ...
    <div className="forum-admin-list">
      {messages.map((message) => (
        <div key={message.id} className="forum-admin-list-item">
          ...
          <span className="forum-admin-list-item-meta">Status: quarantined</span>
          {message.moderationReason ? (
            <span className="forum-admin-list-item-meta">Reason: {message.moderationReason}</span>
          ) : null}
